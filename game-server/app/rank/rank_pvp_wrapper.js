...
rank_pvp_wrapper.prototype.get_rank_info_channel_batch = function(channel, device_guid_array, cb) {
    redis_rank_pvp_wrapper.get_rank_info_channel_batch(channel, device_guid_array, cb);
};
...
rank_pvp_wrapper.prototype.get_score_rank_partial_channel = function(channel, cb) {
    redis_rank_pvp_wrapper.get_score_rank_partial_channel(channel, cb);
};
...
rank_pvp_wrapper.prototype.get_score_rank_partial_activity_channel = function(device_guid, channel, cb) {
    redis_rank_pvp_wrapper.get_score_rank_partial_activity_channel(device_guid, channel, cb);
};
...
rank_pvp_wrapper.prototype.del_channel_rank_info = function(channel) {
    redis_rank_pvp_wrapper.del_channel_rank_info(channel);
};
        ...
        if ("activity" == v) {
            for (var i = 0; i < rank_for_pvp_json[v].length; ++i) {
                if (channel == rank_for_pvp_json[v][i].channel) {
                    ...
                    var time_start = new Date(duration[0]);
                    var time_end = new Date(duration[1]);
...
rank_pvp_wrapper.prototype.calc_rival_pvp_award = function(championship_id) {
    ...
    this.get_all_rank_info_weekly(championship_id, function(reply) {
        ...
        for (var v in rank_joiner_list) {
            if (rank_joiner_list[v]) {
                ...
                if (rank_joiner) {
                    ...
                    device_version_channel_list.push({
                        "device_guid": v,
                        "version": version,
                        "channel": channel
                    });
        ...
        async.whilst(
            function() {
                return count < device_version_channel_list.length;
            },
            function(callback) {
                async.waterfall([
                        function(callback) {
