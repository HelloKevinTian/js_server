...
var charge_feedback_is_filter = function(channel, activity) {
    var filters = activity.filterList;
    var is_filter = false;
    for (var i = 0; i < filters.length; i++) {
        var filter_channel = filters[i].channel;
        var filter_duration = filters[i].duration;

        var start_time_date = new Date(filter_duration[0]);
        var end_time_date = new Date(filter_duration[1]);
        var start_time_stamp = start_time_date.getTime();
        var end_time_stamp = end_time_date.getTime();
        var now_stamp = Date.now();

        if (filter_channel === channel && now_stamp >= start_time_stamp && now_stamp < (end_time_stamp + 3600 * 24 * 1000)) {
            is_filter = true;
        }
    };
    return is_filter;
...
handlerMgr.handler(consts.TYPE_MSG.TYPE_GET_ACTIVITY, function(msg, session, next) {
    ...
    activity_wrapper.get(channel, version, function(activity_json) {
        ...
        } else if (consts.TYPE_ACTIVITY.TYPE_CHARGE_FEEDBACK == type) { //filter channel to close the switch
            if (charge_feedback_is_filter(channel, activity)) {
                activity.switch = "0";
            }
            next(null, {
                code: 0,
        ...
        if (consts.TYPE_ACTIVITY.TYPE_RANDOM_PRIZE == type ||
            ...
            consts.TYPE_ACTIVITY.TYPE_CHARGE_FEEDBACK == type) {
            return;
        }
        next(null, {
            code: 0,
