                    ...
                    if (version === "1.0.2") {
                        ...
                        if (entity_prize_has_win != null) {
                            if (entity_prize_has_win === 0) {
                                prize = get_entity_prize(activity,device_guid,random_prize_the_second_phase_wrapper);
...
function get_entity_prize(activity,device_guid,random_prize_the_second_phase_wrapper){
        ...
        entity_gacha_info.getall(function(reply){
            ...
            if (reply != null) {
                ...
                for(var i = 0; i < reply.length; ++i){
                    ...
                    unit_data.item_type = reply[i++];
                    unit_data.used_num = reply[i];
                    gacha_info.push(unit_data);
                }
            }
            ...
            for (var i = 0;i < entity_unit_type.length;i++) {
                ...
                for (var i = 0; i < gacha_info.length; i++) {
                    if (gacha_info[i].item_type === item_type) {
                        used_prize_num = gacha_info[i].used_num
                    }
                };
                ...
                if (cur_prize_num > 0) {
                    ...
                    if (random_val < get_weight) {
                        ...
                        entity_gacha_winner.set(device_guid.toString() + "_" + now_date.toString(),item_type.toString());     //哪个人抽到哪个奖品存库
                        //log
                        datelogger.debug("winner bingo! :" + device_guid.toString() + "@" + now_date.toString() + "@" + item_type.toString());
                        if (item_type === 5) {
